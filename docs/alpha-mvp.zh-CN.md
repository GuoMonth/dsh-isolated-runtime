...
2026-09-20：按用户确认的[项目宪法](../CONSTITUTION.md)收缩当前里程碑。目标与原则已确定，代码实现和集成验收尚未完成。企业自托管是方向，不代表当前生产可用。
...
- 不做 HA、多集群、自动扩缩容、备份灾备、通用 runtime 服务、Process/Docker 新后端、安装发行产品扩张。已有能力不是新增兼容义务，仍相关的隔离/ownership 检查要保留。
