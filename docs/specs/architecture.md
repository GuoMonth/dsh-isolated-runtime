...
> **Design target, not current implementation.** The authoritative design and sequence are in the [platform AgentEnvironment design](https://github.com/GuoMonth/dsh-multi-tenant/blob/main/docs/design/agent-environment.zh-CN.md) and [Issue #104](https://github.com/GuoMonth/dsh-multi-tenant/issues/104). Current source and released artifacts still use `Cell`; no new product Kind is promised at this stage. W1 removes Process/Docker product backends, standalone launch, and snapshot/restore. Runtime #100 has passed the bounded upstream live check; the production adapter is still to be implemented. Do not infer W1/W2/W3 are complete or released.

## Ownership

An AgentEnvironment is the planned product boundary for one agent instance and its persistent HOME/data. Kubernetes is the only product execution backend. W1 adopts the upstream core `Sandbox` CRD/controller as an infrastructure object with an ordinary Pod and external PVCs; no synonym product CRD or fork is used. The platform/runtime boundary isolates user identity and product protocols from infrastructure mechanics. Namespace is administrator-configured infrastructure scope, not OIDC tenant identity; the reference setup preconfigures per-user namespaces rather than creating an automatic namespace tenancy system. Platform authorization is the sole user-owner authority; runtime stores immutable owner linkage only for resource matching and does not implement OIDC. See the [bounded live evidence](https://github.com/GuoMonth/dsh-multi-tenant/blob/main/docs/evidence/agent-sandbox-local-2026-09-23.md).
...
W2 adds explicit healthy-node normal stop/start on the upstream implementation, retaining environment and both PVC identities. Product `Running`/`Stopped` maps to `Running`/`Suspended` intent, with verified stop evidence rather than blindly copying upstream status. Production must retain cross-resourceVersion concurrency, unknown deletion barriers and exact old-UID checks. Stopping retains both volumes. The current Cell path remains the production implementation until W1 lands. No node-partition fencing is promised; force deletion on an unhealthy or partitioned node is outside any stop guarantee. Backups, hot pools and automatic idle are deferred. W3 requires joint OIDC authorization, real tools, persistent HOME and release validation before publication.
