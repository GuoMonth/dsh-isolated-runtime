...
Keep the existing Pod baseline: non-root, no privilege escalation, dropped capabilities, RuntimeDefault seccomp, no mounted ServiceAccount token, host directories or Docker socket. The fixed template sets `DSH_PERMISSION_MODE=danger-full-access` only for DSH's nested tool permission layer because the Cell image has no `bwrap`; this does not grant permissions beyond the existing Pod boundary or alter its OS/container, filesystem, token or network controls. CPU/memory requests and limits are explicit in the fixed Cell template; cluster policy must not mutate them. Generated policies primarily cover Cell ingress; administrators configure and verify unnecessary control-plane/private-network egress restrictions in the reference deployment. Pod existence alone does not enforce those restrictions.
