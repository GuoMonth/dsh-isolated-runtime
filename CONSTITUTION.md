...
- 架构目标是 Kubernetes 唯一后端的 Agent Workspace。薄 CRD/Operator 聚合工作区资源；目标 Kind 为 `AgentWorkspace`，由 W1 破坏性改名，当前源码仍实现 `Cell`。不承诺 Process/Docker 产品运行后端或多后端兼容。
...
- 平台与 runtime 保持职责隔离，但不要求平台直接管理 Kubernetes 对象：runtime 可以通过薄资源契约管理原生 StatefulSet、PVC、Service 与策略。
