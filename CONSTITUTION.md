...
- 架构目标是 Kubernetes 唯一后端的 AgentEnvironment。上游接入检查使用基础设施 `Sandbox`，不承诺新产品 Kind；当前源码仍实现 `Cell`。不承诺 Process/Docker 产品运行后端或多后端兼容。
