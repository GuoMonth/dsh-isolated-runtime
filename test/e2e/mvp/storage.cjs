...
function walk(directory) {
  for(const entry of fs.readdirSync(directory,{withFileTypes:true})) {
    ...
    if(entry.isDirectory())walk(file);
      ...
      if(file.includes('/attachments/v1/objects/'))images.push(data);
...
if(mode==='deterministic') {
  // DSH normalizes grayscale PNGs to sRGB; compare decoded pixels, not encoding.
  ...
  const expected=await sharp(original).toColourspace('srgb').ensureAlpha().raw().toBuffer();
  let readable=false;
  for(const image of images) {
    const decoded=await sharp(image).toColourspace('srgb').ensureAlpha().raw().toBuffer({resolveWithObject:true});
    if(decoded.info.width===1&&decoded.info.height===1&&decoded.data.equals(expected))readable=true;
  }
  assert(readable,'uploaded image pixels missing or changed');
}
