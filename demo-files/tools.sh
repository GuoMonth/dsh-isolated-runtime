...
prepare_demo_tools() {
  ...
  if ! command -v kind >/dev/null || [[ "$(kind version)" != *v0.32.0* ]]; then
    curl -fsSL --retry 3 https://kind.sigs.k8s.io/dl/v0.32.0/kind-linux-amd64 -o "$demo_root/tools/bin/kind"
    curl -fsSL --retry 3 https://kind.sigs.k8s.io/dl/v0.32.0/kind-linux-amd64.sha256sum | awk '{print $1 "  kind"}' > "$demo_root/tools/bin/kind.sha256"
  ...
  if ! command -v kubectl >/dev/null || [[ "$(kubectl version --client -o json)" != *v1.34.0* ]]; then
    curl -fsSL --retry 3 https://dl.k8s.io/release/v1.34.0/bin/linux/amd64/kubectl -o "$demo_root/tools/bin/kubectl"
    printf '%s  kubectl\n' "$(curl -fsSL https://dl.k8s.io/release/v1.34.0/bin/linux/amd64/kubectl.sha256)" > "$demo_root/tools/bin/kubectl.sha256"
    (cd "$demo_root/tools/bin" && sha256sum -c kubectl.sha256)
    chmod 755 "$demo_root/tools/bin/kubectl"
  fi
  if ! command -v jq >/dev/null; then
    curl -fsSL --retry 3 https://github.com/jqlang/jq/releases/download/jq-1.8.1/jq-linux-amd64 -o "$demo_root/tools/bin/jq"
    curl -fsSL https://github.com/jqlang/jq/releases/download/jq-1.8.1/sha256sum.txt | awk '$2=="jq-linux-amd64" {print $1 "  jq"}' > "$demo_root/tools/bin/jq.sha256"
  ...
  if ! command -v node >/dev/null || [[ "$(node --version)" != v24.18.0 ]]; then
    curl -fsSL --retry 3 https://nodejs.org/dist/v24.18.0/node-v24.18.0-linux-x64.tar.xz -o "$demo_root/tools/node.tar.xz"
    curl -fsSL https://nodejs.org/dist/v24.18.0/SHASUMS256.txt | awk '$2=="node-v24.18.0-linux-x64.tar.xz" {print $1 "  node.tar.xz"}' > "$demo_root/tools/node.sha256"
