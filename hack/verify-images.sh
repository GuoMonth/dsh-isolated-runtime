...
mkdir -p "$smoke_root/data" "$smoke_root/private"
chmod 0777 "$smoke_root/data" "$smoke_root/private"

start_cell() {
  docker run --detach --name "$container_name" \
    --read-only --security-opt no-new-privileges --cap-drop ALL \
    --tmpfs /tmp:rw,noexec,nosuid,nodev,uid=1000,gid=1000,size=1g \
  ...
  for _ in $(seq 1 90); do
    if curl --silent --show-error --fail "http://127.0.0.1:$management_port/readyz" >/dev/null 2>&1; then
      curl --silent --show-error --fail "http://127.0.0.1:$management_port/version" \
        | jq -e '.contractVersion == "v1alpha1" and .dshVersion == "0.1.2-alpha.4"' >/dev/null
      return
    fi
    sleep 1
  done
