...
set -Eeuo pipefail
run_id="${1:?candidate run id}"; output="${2:?output directory}"; target="${3:-linux-amd64}"
[[ "$run_id" =~ ^[0-9]+$ ]]
...
mkdir -p "$output"
gh run download "$run_id" --repo GuoMonth/dsh-isolated-runtime --name "mvp-candidate-$target" --dir "$output"
