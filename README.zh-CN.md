...
**当前状态：Phase 0 契约已完成。** 仓库包含 Cell API、生成的 CRD、DSH 兼容实证以及
可执行的 access seam 原型；生产级 controller 与镜像有意留给 Phase 1 的垂直切片。
...
namespace 就是租户边界。镜像必须固定 digest；存储只能扩容，`storageClassName` 不可变。
