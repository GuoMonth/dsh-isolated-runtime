...
**Architecture direction:** the next design is Kubernetes-only Agent Workspace; current source/packages still use Cell. W1 plans a breaking `AgentWorkspace` rename and removal of Process/Docker product backends, standalone launch, and snapshot/restore. See the [authoritative design](https://github.com/GuoMonth/dsh-multi-tenant/blob/main/docs/design/agent-workspace.zh-CN.md) and [Issue #104](https://github.com/GuoMonth/dsh-multi-tenant/issues/104). This direction does not remove Pod child processes, OCI image builds, or Docker as kind's substrate.
